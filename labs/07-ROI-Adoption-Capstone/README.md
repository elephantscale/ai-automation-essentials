...
Open by showing one complete capstone. Use the support-triage workflow if you want continuity,
or the vendor-invoice workflow if you want variety after several support examples.

Recommended timing:

| Activity | Minutes |
|----------|---------|
| Worked capstone demo | 15 |
| Workflow selection and scope check | 15 |
| Enterprise setup + design canvas | 20 |
| Build in ChatGPT | 35 |
| Test cases + review queue | 20 |
| Connector readiness + ROI | 25 |
| Presentation prep | 10 |
| Presentations and feedback | 30 |

Scope-check sentence:

```text
One trigger, one output, one human gate, one measurable benefit.
```

Model capstone: vendor invoice exception queue

```text
Trigger today: upload sample-vendor-invoices.csv.
AI task: clean categories, normalize dates/amounts, flag missing values, duplicates, and credits.
Output: exception queue and spend summary.
Human gate: finance analyst reviews flagged rows before any payment or accounting update.
Connector future: read-only accounting export first; no write-back until policy and controls exist.
ROI: minutes saved per invoice review run, reduced rework, conservative maintenance cost.
```

This model adds domain variety and avoids making every final project another support ticket
triage flow.
