...
Run this as a guided first lab. Students can follow along, but you drive the pace.

Recommended timing:

| Activity | Minutes |
|----------|---------|
| Open dataset and frame the task | 5 |
| Rules-engine pass in ChatGPT | 8 |
| AI interpretation pass in ChatGPT | 8 |
| Compare rows | 12 |
| Hybrid workflow sentence | 7 |
| Debrief | 5 |
...
```text
We are not proving that rules beat AI. We are proving that automation has two different
tools: exact rules for stable signals, and AI for messy language.
```

After the rules pass:

```text
Notice the rules engine is not bad. It did exactly what we told it to do. It routed obvious
keyword cases quickly and consistently. But it missed requests whose meaning was clear to us
because the expected keyword was absent.
```

After the AI pass:

```text
AI filled in the interpretation gap. It saw that "can't get into my workspace" is an account
support issue and "options for 50 users" is a sales conversation. That is the value: not magic,
just better interpretation of messy language.
```

Land here:

```text
In a real automation, I would not use keyword rules as the main intelligence. I would use them
for obvious cheap routing, then hand the ambiguous or unmatched middle to AI.
```
...
This version is intentionally easier to run than the original. The instructor no longer has to
make students manually fill the rules table. ChatGPT simulates both passes, but the first pass
is constrained to exact rules. That keeps the lab moving and makes the teaching point clearer.
...
4 Technical Support
5 Sales
6 Implementation / Automation Help
```
...
| Row | Teaching Point |
|-----|----------------|
| 1 | Rules are enough when the expected words are present. |
| 2 | AI understands access trouble even without `login` or `password`. |
| 3 | AI understands buying intent even without `demo`, `pricing`, or `quote`. |
| 4 | Rules are enough for a clear technical keyword. |
| 5 | Rules are enough for a clear sales keyword. |
| 6 | AI handles a new category the rules did not anticipate. |

### One Idea To Land

```text
Rules are useful for stable operational signals. AI is useful when language varies.
Automation design is deciding where each belongs.
```
